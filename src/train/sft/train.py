...
def instance_process_func(example):
    ...
    # TODO：回车label不是-100，好像有点问题。后续需要对一下LLAMAFactory的token id list
    for term in example['text']:
        #print('@', term)
        prefix_tokens = tokenizer(IM_START+ROLE_TOKEN[term['role']], add_special_tokens=False)
        content_tokens = tokenizer(term['content'], add_special_tokens=False)
        suffix_tokens = tokenizer(IM_END, add_special_tokens=False)

        input_str += IM_START+ROLE_TOKEN[term['role']] + term['content'] + IM_END
        input_ids += prefix_tokens['input_ids']+content_tokens['input_ids']+suffix_tokens['input_ids']
        attention_mask += prefix_tokens['attention_mask']+content_tokens['attention_mask']+suffix_tokens['attention_mask']
        labels += [-100] * len(prefix_tokens['input_ids'])
        if term['role'] == 'assistant':
            labels +=  content_tokens['input_ids'] + suffix_tokens['input_ids']
        else:
            labels += [-100] * len(content_tokens['input_ids']+suffix_tokens['input_ids'])
